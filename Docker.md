...
3. Tmpfs mounts: A piece of RAM that can be mounted to a container as a folder. Files created by the container in this folder are readonly and will exist as long as the container does. As opposed to volumes, tmps mounts cannot be share between containers.
  ...
  When designing docker based containers as a service (CaaS), prioritizing centralizing logging is crutial due to the number of logs or events produced by them. The logs are necessary to use typically but not limited for troubleshooting and debugging. 

  There are two ways to work with logs in containers:

  
1. Check logs of containers by using `docker logs` command. This command shows the SDTOUT and STDERR of processes/commands/applications running inside the container.

    - `$ docker logs --follow mycontainer` Shows the logs generated by mycontainer.
    - `$ docker logs --tail N <date> mycontainer` Shows the last N log lines generated by mycontainer
    - `$ docker logs --timestamp <date> mycontainer` Shows each logs generated with timestampt
    - `$ docker logs --since <date> mycontainer` Shows the logs generated by mycontainer since an specific date

2. Configure docker daemon and container to use some log driver. By default **json-file** logging driver is used in docker, this logging driver stores the logs of containers in **/var/lib/docker/containers**. Other logging drivers migth send the log files directly to centralized logging servers.
    
    - None: Does not store logs of containers.
    - Json-file: Stores logs in form of json files in the host machine. By default this driver does not provide log rotation policy which may lead to exhasution of disk space.
    - Syslog: Uses syslog deamon of the host machine to process containers logs
    - Jounarld: Uses journal daemon of the host machine to process containers logs
    - Awslogs: Can send logs to aws cloudwatch
    - Spluk: Can sen logs to splunk service.

    To change the logging driver used by docker and the containers, the file **/etc/docker/daemon.json** file needs to be configured. After that, the daemon needs to be restarted to apply new configurations. Existing running containers won´t change to new loggin driver, they must be restarted.

    - `$ docker run --log-driver journald myimage` Runs a container with the specified log driver.
